...
console.log("Supplier catalog smoke");
{
  ...
  const priceItems = [
    { furnitureType: "kitchen", material: "premium", basePriceKzt: 20000, unitPriceKzt: 55000, unit: "м.п." },
    { furnitureType: "kitchen", material: "standard", basePriceKzt: 15000, unitPriceKzt: 45000, unit: "м.п." },
    { furnitureType: "wardrobe", material: "standard", basePriceKzt: 10000, unitPriceKzt: 35000, unit: "м.п." },
    { furnitureType: "other", material: "standard", basePriceKzt: 8000, unitPriceKzt: 30000, unit: "м.п." }
  ];
...
console.log("Supplier store smoke");
{
  ...
  const supplierResult = await createSupplier({ db, code: "SUP001", name: "ТОО Мебельные Компоненты", contact: "Иван", phone: "+7 700 111 22 33", email: "info@mk.kz" });
  ...
  const priceListResult = await createPriceList({ db, supplierId, note: "Первый прайс-лист" });
  ...
  const itemResult = await addPriceItem({ db, priceListId, supplierId, furnitureType: "kitchen", material: "premium", label: "Кухня премиум", basePriceKzt: 20000, unitPriceKzt: 55000 });
  ...
  const item2 = await addPriceItem({ db, priceListId, supplierId, furnitureType: "kitchen", material: "standard", label: "Кухня стандарт", basePriceKzt: 15000, unitPriceKzt: 45000 });
  ...
  const v2 = await createPriceList({ db, supplierId, note: "Обновленный прайс" });
  assertEqual(v2.body.item.versionNumber, 2, "second price list version 2");

  const archiveResult = await archivePriceList({ db, priceListId });
...
console.log("Supplier-aware estimate smoke");
{
  ...
  const supplier = await createSupplier({ db, code: "SUP002", name: "Фурнитура Плюс" });
  ...
  const draft = await createPdfDraft({
    ...
    manifest: {
      ...
      pageCount: 1,
      pages: [{ pageNumber: 1, pageType: "floor_plan", furnitureZones: [
        { id: "z1", zoneType: "kitchen", label: "Кухня 3.2м", dimensions: { widthMm: 3200 } },
        { id: "z2", zoneType: "wardrobe", label: "Шкаф 2.5м", dimensions: { widthMm: 2500 } }
      ]}]
    }
  });
